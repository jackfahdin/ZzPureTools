...
#include "ZzTheme.h"
#include "backend/ZzDemoViewModel.h"
    ...
    , m_viewModel(new ZzDemoViewModel(this))
{
    ...
    if (ZzTheme* theme = ZzApplication::instance().theme()) {
        auto applyWindowColors = [this, theme]() {
            QPalette pal = palette();
            pal.setColor(QPalette::Window, theme->colors().windowBase);
        ...
        applyWindowColors();
        connect(theme, &ZzTheme::colorsChanged, this, applyWindowColors);
    }
}
...
void MainWindow::bindViewModel()
{
    ...
    connect(m_viewModel, &ZzDemoViewModel::themeModeApplyRequested, this, [](ZzThemeMode mode) {
        if (ZzApplication::instance().theme() != nullptr) {
            ZzApplication::instance().theme()->setMode(mode);
        }
    });
