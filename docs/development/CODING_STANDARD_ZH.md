...
- 有动画状态的控件必须按实例固定持有动画对象；允许首次需要时延迟创建，但创建后必须复用，禁止在每次 hover、选择、展开、关闭或反向操作时重复分配，也禁止用 `DeleteWhenStopped` 代替明确的生命周期管理。
- 快速连续操作必须先读取当前显示状态，再以该状态为起点重定向同一个动画；不得先跳回旧终态或初始态。页面切换动画必须可取消，异步回调使用 `QPointer` 或 QObject context 守护生命周期。
- reduced motion 生效时必须停止正在运行的非必要动画并同步到当前请求的终态，不得依赖一次极短动画模拟同步完成。隐藏、禁用、模型重置和主题切换路径同样需要留下确定、可绘制的状态。
- 动画测试必须断言每个 owner 的动画对象地址和数量预算在快速操作后保持不变；涉及额外定时器、事件过滤器或 QObject 子对象时，还必须证明其数量有界且销毁后无残留。
