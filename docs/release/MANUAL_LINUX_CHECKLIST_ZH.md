...
| [ ] Activity 四区域菜单 | 分别在 Left/Right Primary/Secondary 打开入口菜单；只出现其余三个合法目标，当前区域不出现，禁用目标不被接受 |  |  |  |
| [ ] 同侧 Single 切换与折叠 | 在同一物理侧连续切换两个入口时始终只显示一个 Side 页面；再次激活当前入口折叠，再次激活恢复原宽度 |  |  |  |
| [ ] 右侧空态与恢复 | 移除最后一个右侧入口后 Activity 和 Side 都不占宽；重新注册后在原物理右边恢复，活动项只出现一条短指示 |  |  |  |
| [ ] RTL 物理边 | RTL 下 Left/Right 入口、Side 和活动短指示仍位于声明的物理左/右边，文字与键盘顺序按 RTL 展示 |  |  |  |
| [ ] 设置窗口模态与复用 | 从固定设置入口打开逐窗口 `WindowModal` 设置；重复触发只 raise/activate 同一实例，关闭后可重建 |  |  |  |
| [ ] 设置多窗口隔离 | 两个主窗口各自只拥有一个设置窗口；关闭或激活其中一个不影响另一个的模态、焦点和生命周期 |  |  |  |
| [ ] 工作区后端组合 | 在 X11、Wayland 与 `ZZ_WINDOWKIT_FORCE_QT_CONTEXT` fallback 分别复验六入口、右侧空态恢复、物理边指示和设置窗口 |  |  |  |
