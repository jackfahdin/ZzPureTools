...
- 综合示例已经接入独立的启动、空闲、页面切换、主题切换和十万行模型场景，
  使用既有 reporter schema、环境指纹和绝对门限；普通示例构建不编译测量控制器。
  当前仍需在本批代码提交后的干净 HEAD 上按固定 Xvfb 与 CPU 亲和条件采集五份
  原始 JSON，完成相对基线前不得用本轮功能测试报告或既有组件报告冒充最终结果。
