...
- 本计划已于 2026-08-11 完成实施与 Linux 参考机验收，后续内容保留为接口和回归依据。
...
| 序号 | 提交 | 内容 |
|---|---|---|
| 1 | `9b79f65` | 细化第 0、1 批实施路线 |
| 2 | `6c568df` | 建立逐指标噪声阈值门禁 |
| 3 | `c4531b5` | 禁止新增裸颜色与关键尺寸魔数 |
| 4 | `f96015b` | 补充反馈绘制原语与主题令牌 |
| 5 | `1afa250` | 新增信息徽章组件 |
| 6 | `2152066` | 新增内容对话框组件 |
| 7 | `cd5df32` | 新增教学提示组件 |
| 收尾 | `a6e7459` | 消除反馈组件 Clang-Tidy 诊断 |
| 门禁 | `9f192f3` | 稳定 Fontconfig 字体场景的 LSan 检查 |
| 8 | 本次文档提交 | 完成第一批验收与路线状态同步 |

每个提交必须先通过对应定向测试；影响共享行为后再跑完整 Linux reference。未执行的平台不得写成已验证。

## 6. 实际验收结果

验收基于代码提交 `9f192f3`，使用 Qt 6.11.1、GCC 15.2.0、Clang 20.1.8 和项目本机发布参考档案：

| 门禁 | 结果 |
|---|---|
| Linux reference | `159/159` 通过，其中包含 38 个 benchmark 和 15 个 reference gate |
| Clang shared/static | 两档严格构建和 `122/122` CTest 通过；197 个一方文件均为零诊断 |
| ASan/UBSan | 完整 `122/122` 通过；窗口生命周期与导航压力场景 `2/2` 通过 |
| 性能相对门 | `linux-gcc-benchmarks` 为 `144/144`；固定 CPU 8/10 后 12 个场景全部通过逐指标比较 |
| 视觉 | Fluent 与综合示例的 100/125/150/200 四档截图全部通过；ContentDialog 和 TeachingTip 各新增 12 张独立三主题基线，InfoBadge 纳入通用三主题基线 |
| 安装与架构 | 安装消费、包重定位、公开头、二进制依赖和视觉令牌扫描全部通过 |

性能验收期间动画 `frame-time.max` 曾出现一次 18.56ms 调度尖峰；同一固定环境复测和最终 reference 均未复现，未修改基线、阈值或 `gate/observe` 策略。

Windows MSVC、Windows MinGW 与 macOS arm64/x86_64 在本批收尾阶段均未执行，因此不登记 shared/static 构建通过，也不登记真机交互通过。公共 API、平台分支和 CMake preset 已保留静态设计边界，后续结果必须由对应环境补录。
