...
```bash
cmake --build --preset linux-gcc-debug --target ZzFluentScreenshotTest ZzBasicControlsBenchmark --parallel 2
ctest --preset linux-gcc-debug -R 'fluent\\.(screenshot|basic-controls-benchmark)' --output-on-failure
```
...
- [ ] **步骤 3：运行 clang-tidy 目标和基准**

运行：

```bash
cmake --build --preset linux-gcc-debug --target ZzClangTidy --parallel 2
ctest --preset linux-gcc-debug -R 'fluent\\.basic-controls-benchmark' --output-on-failure
```
