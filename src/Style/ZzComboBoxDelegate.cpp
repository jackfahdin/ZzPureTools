...
void ZzComboBoxDelegate::paint(QPainter& painter, const QRectF& rect,
                               const ZzStyleContext& context) const
{
    ...
    ZzPaintPrimitives::drawComboArrow(painter, rect, colors,
                                      context.enabled ? context.state : ZzControlState::Disabled);

    const QRectF textRect(rect.left() + metrics.contentPadding, rect.top(),
                          rect.width() - metrics.contentPadding - 28.0, rect.height());
    painter.save();
...
ZzComboBoxItemDelegate::ZzComboBoxItemDelegate(QObject* parent) : QStyledItemDelegate(parent) {}

void ZzComboBoxItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
                                   const QModelIndex& index) const
{
    ...
    if (fill.alpha() > 0) {
        QPainterPath path;
        path.addRoundedRect(rect.adjusted(4, 1, -4, -1), metrics.controlRadius - 1,
                            metrics.controlRadius - 1);
        painter->fillPath(path, fill);
    }

    painter->setPen(theme->textColor(state));
    painter->drawText(rect.adjusted(metrics.contentPadding, 0, -8, 0),
                      Qt::AlignVCenter | Qt::AlignLeft, index.data(Qt::DisplayRole).toString());
    painter->restore();
}

QSize ZzComboBoxItemDelegate::sizeHint(const QStyleOptionViewItem& option,
                                       const QModelIndex& index) const
{
    Q_UNUSED(index)
    ZzTheme* theme = ZzApplication::instance().theme();
